...
repository root (it is git-ignored). `read_config()` loads it and reads
[`inst/config.yml`](inst/config.yml), which expects:
...
`validate_wf_surveys()`. The main workflow runs them in this order, with
the GPS steps running alongside the survey steps:
...
[`data-pipeline.yaml`](.github/workflows/data-pipeline.yaml) runs at
00:00 UTC every fourth day of the month (1st, 5th, 9th, …) and on every
push. Its first job builds the Docker image from `Dockerfile.prod` with
the latest `coasts` release; every other job runs inside it. Runs on
`main` use the `production` profile; runs on any other branch use
`default`.

### Releases

Bump `Version:` in `DESCRIPTION` and add a block at the top of
[`NEWS.md`](NEWS.md) headed `# peskas.zanzibar.data.pipeline X.Y.Z`,
written for non-technical readers. On push to `main`, `release.yml`
turns that block into a GitHub release if the version is new.
...
Read [`.github/CONTRIBUTING.md`](.github/CONTRIBUTING.md). Code follows
the [tidyverse style guide](https://style.tidyverse.org); comment
`/style` on a pull request to apply it. New to R packages? See [*R
Packages*](https://r-pkgs.org) by Hadley Wickham and Jenny Bryan. For
