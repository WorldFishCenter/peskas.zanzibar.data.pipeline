...
#' catch/general info using precomputed length-weight coefficients.
#'
#' @param catch_info Processed catch information for one form version
#' @param general_info Processed general information for one form version
#' @param lwcoeffs Precomputed length-weight coefficients (list with `lw` and
#'   `ml` elements). Computed once in `preprocess_wf_surveys()` across the
#'   union of taxa from all available versions, then reused for every version.
#'
#' @return Combined and processed survey data for one version
#' @keywords internal
process_version_data <- function(catch_info, general_info, lwcoeffs) {
  catch_df <-
